...
- **含巢狀表格的儲存格不再每次存檔多長一個段落**（#155）。`TableCell.toXML()` 把儲存格攤平成
  「全部段落 → 全部巢狀表格 → 一個新的 `<w:p/>`」。原本在表格**之後**的那個尾端段落因此被排到表格
  **之前**，然後又補一個新的——每存一次、每個含巢狀表格的儲存格 +1，**線性且無上限**。觸發條件只是
  任一 typed-dirty 操作（`markTypedDirty("word/document.xml")`），不需要動到表格本身。

  本機 653 份 .docx 語料庫裡有 3 份含巢狀表格，修復前**三份全部**在四代存檔中持續成長、修復後三份
  全部原樣往返：

  | 文件 | 3.7.0 | 修復後 |
  |---|---|---|
  | 兼職揭露告知書（2 個巢狀表格儲存格）| 12 → 14 → 16 → 18 | 12 → 12 → 12 → 12 |
  | REC-P-011 新案審查申請書 | 38 → 40 → 42 → 44 | 38 → 38 → 38 → 38 |
  | blk-paras-and-tables | 4 → 6 → 8 → 10 | 4 → 4 → 4 → 4 |

  修法是**回收**儲存格自己的最後一個段落到表格之後，不再另外補一個。合法的儲存格必然以段落結尾
  （Word 無條件寫一個），所以攤平後的最後一個段落就是原本在表格之後的那個——不論它是不是空的。

  **先試過「只回收空段落」，不夠**：真實表單的尾端段落帶文字，於是第一次存檔仍然每個儲存格 +1
  （12 → 14），第二次才停。有上限了，但文件已經被改動過。這條記在這裡是因為結論本身看不出這件事。

  **仍然做不到的**：一個儲存格裡有**數個**巢狀表格時，它們之間原本夾著的段落仍會被排到全部表格之前。
  那需要 typed model 不再是序列化的權威（#133 / #129），不在本次範圍。舊程式碼的註解寫「Word 會在缺少
  時補一個」——Word 確實如此，而這段程式從來沒有檢查過。
