...
- **表格儲存格裡的段落與巢狀表格不再被重排，也不再每次存檔多長一個**（#155）。

  根因在**讀取端**：`parseTableCell` 分兩趟掃儲存格——先收所有 `<w:p>`、再收所有 `<w:tbl>`——
  **交錯順序在解析當下就被丟掉**，`paragraphs` 與 `nestedTables` 兩個陣列之間再也答不出
  「這個段落在表格前還是表格後」。寫入端的攤平只是照抄這個攤平，於是同時發生兩件事：

  1. 原本在表格**之後**的尾端段落被排到表格**之前**，然後又補一個新的 `<w:p/>`——
     每次 typed-dirty 存檔、每個含巢狀表格的儲存格 **+1，沒有上限**（實測 12 → 14 → 16 → 18）。
  2. **重排本身就搬動了使用者內容**。`A, 表格, B, C` 重新輸出成 `A, B, C, 表格, <w:p/>`。

  修法是讓讀取端**只掃一趟、按文件順序記下 block 序列**（`TableCell._blocks`），寫入端照該
  順序輸出。tree-backed 的儲存格改走 `xmlNode.children`，同一個道理。沒有記錄到順序的儲存格
  （呼叫端自己組的，或 `paragraphs` / `nestedTables` 被整批取代過的）維持舊形狀——那裡本來就
  沒有順序可保存，並且不會進入成長迴圈。

  本機 653 份 .docx 語料庫中 **6 份**含巢狀表格儲存格。修復前六份全部持續成長，速率是
  **每個儲存格每次存檔 +1**；修復後六份的儲存格區塊順序**四代都與原始檔的 `word/document.xml`
  逐項相同**：

  | 文件 | 巢狀表格儲存格 | 3.7.0 段落數 | 修復後 |
  |---|---|---|---|
  | 兼職揭露告知書 | 1 | 6 → 7 → 8 → 9 | 6 → 6 → 6 → 6 |
  | REC-P-011 新案審查申請書 | 1 | 19 → 20 → 21 → 22 | 19 → 19 → 19 → 19 |
  | blk-paras-and-tables | 1 | 2 → 3 → 4 → 5 | 2 → 2 → 2 → 2 |
  | 新潮式表格履歷 | 3 | 4 → 7 → 10 → 13 | 4 → 4 → 4 → 4 |
  | 新潮式履歷表（作業四）| 5 | 5 → 10 → 15 → 20 | 5 → 5 → 5 → 5 |
  | 履歷簽名 | 5 | 5 → 10 → 15 → 20 | 5 → 5 → 5 → 5 |

  **這張表先前兩個版本都是錯的，錯法值得記下來**，因為三次都出在量測而不是修法：

  1. 第一版用「儲存格段落總數不變」當證據（12 → 12 → 12 → 12）。**看不見這個缺陷**——把段落
     搬到表格另一側，數量完全不變。中間那版「保留最後一個段落」的錯誤修法正是這樣通過的。
  2. 第二版改成比對**輸出的儲存格 XML**，但比的是「第 N 代 vs 第 0 代**自己的輸出**」。那測的是
     **穩定性**，不是**忠實度**：一個把內容搬動一次、之後就不再變的輸出是穩定的。把順序記成
     「所有表格、然後所有段落」的突變照樣通過。
  3. 兩版的走訪都同時從 `body.tables` 與 `body.children` 播種，而兩者暴露的是**同一批表格**，
     於是每個儲存格被數兩次、段落數整體加倍（表中「12 → 14 → 16 → 18」其實是 6 → 7 → 8 → 9），
     而且用堆疊（`popLast`）走訪讓輸出清單與來源清單順序相反，把儲存格 1 拿去跟儲存格 4 比。
     文件數也因此少報：實際是 6 份不是 3 份。

  現在的測試比對的是**原始 .docx 的 `word/document.xml` 位元組裡的區塊順序**——與 typed model
  完全無關的基準——並斷言比較次數大於零。
  ...
  **一個修復途中引進、由 R2 審查抓到的缺陷，記在這裡**：判斷「至少要有一個段落」時數的是**輸出的 block 數**
  ...
  給出不同答案。現在兩處都以段落計數，且補上的段落一定在最後——那正是 `<w:tc>` 要求的位置。這**不會**
  重啟成長：補段落只在儲存格原本就不以段落結尾時發生，補完再讀就記進 `_blocks`，下一次存檔照原樣輸出。

  仍然做不到的：儲存格裡的 block-level SDT、書籤標記、修訂範圍等沒有 typed 表示的子元素照舊被
  丟棄——這在本次修改前後相同，屬於 #133 / #129 的範圍，未在此擴大處理。
