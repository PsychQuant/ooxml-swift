...
- **表格儲存格裡的段落與巢狀表格不再被重排，也不再每次存檔多長一個**（#155）。
  ...
  **交錯順序在解析當下就被丟掉**，`paragraphs` 與 `nestedTables` 兩個陣列之間再也答不出
  ...
  本機 653 份 .docx 語料庫中 3 份含巢狀表格。修復前三份全部在四代存檔中持續成長，修復後
  三份的**儲存格 XML 四代逐字元相同**：

  | 文件 | 3.7.0 段落數 | 修復後 |
  |---|---|---|
  | 兼職揭露告知書（2 個巢狀表格儲存格）| 12 → 14 → 16 → 18 | 四代逐字元相同 |
  | REC-P-011 新案審查申請書 | 38 → 40 → 42 → 44 | 四代逐字元相同 |
  | blk-paras-and-tables | 4 → 6 → 8 → 10 | 四代逐字元相同 |

  **兩個先前寫錯的說法，在這裡更正**（原文出自本次修復途中的中間版本，未曾發布）：

  - 曾寫「只有**數個**巢狀表格的儲存格會失去交錯」。**錯**——`A, 表格, B, C` 這種
    只有**一個**表格的普通儲存格就會被搬動。現已修正，並有測試鎖住。
  - 曾以「儲存格段落數四代不變（12 → 12 → 12 → 12）」當作修好的證據。**那個量測看不見這個缺陷**：
