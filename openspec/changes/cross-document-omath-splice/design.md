...
[**Risk: a vendor element with the same local name loses generic raw replacement semantics**] → Mitigation: the root classifier rejects an explicitly declared namespace unless it is the standard OMML URI. A quote-aware root attribute tokenizer distinguishes real namespace attributes—including empty undeclarations—from namespace-looking text inside values. Parsing/injection support prefixed/default namespaces, valid dotted prefixes, both quote styles, and nested shadowing. Non-OMML regression fixtures assert exact `Run.rawXML` replacement; standard/inherited OMML fixtures receive the Run carrier and survive splice→write→reload (#117).
