...
[**Risk: a vendor element with the same local name loses generic raw replacement semantics**] → Mitigation: the root classifier rejects an explicitly declared namespace unless it is the standard OMML URI. Namespace parsing reads only the root QName and root start tag, supports prefixed and default namespaces, accepts both XML-legal quote styles, and ignores nested namespace shadowing. Non-OMML regression fixtures assert exact `Run.rawXML` replacement through both Run and Paragraph serializers, while standard/inherited OMML fixtures still receive the Run carrier (#117).
