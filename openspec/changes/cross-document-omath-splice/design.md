...
[**Risk: a vendor element with the same local name loses generic raw replacement semantics**] → Mitigation: the root classifier rejects an explicitly declared namespace unless it is the standard OMML URI. Namespace parsing and self-contained declaration injection read only the root QName/start tag, support prefixed/default namespaces and valid dotted prefixes, accept both XML-legal quote styles, and ignore nested namespace shadowing. Non-OMML regression fixtures assert exact `Run.rawXML` replacement through both Run and Paragraph serializers; standard/inherited OMML fixtures receive the Run carrier and survive splice→write→reload (#117).
