...
///
// MARK: - Compile gate (lifted v0.31.0)
//
// Historical context (preserved per `paragraph-tree-projection-impl` Decision 6):
// this test class was originally gated behind `#if false` (commit 8d3dd49) so
// the test target could build before `Paragraph(xmlNode:)` and `paragraph.id`
// existed in `Sources/OOXMLSwift/Models/Paragraph.swift`. The Spectra change
// `paragraph-tree-projection-impl` lands the production implementation and
// removes the gate; the explanatory comment stays so future readers know why
// the gate ever existed.
